var dumpServiceNotes = function(noteRegex, ip) {
  ...
  ports = Ports.find({'project_id': PID, 'notes': {$elemMatch: {'title': {$regex: noteRegex, $options: 'i'}}}}, {notes: 1, host_id: 1}).fetch();
  hostIds = _.pluck(ports, 'host_id');
  hosts = Hosts.find({'_id': {$in: hostIds}}, {sort: {long_addr: 1}, string_addr: 1}).fetch();
  hosts.forEach(function(host) {
      if(ip !== "" && ip !== host.string_addr) {
          return;
      }
      ports = Ports.find({'host_id': host._id}, {sort: {port: 1}, notes: 1, port: 1, protocol: 1}).fetch();
