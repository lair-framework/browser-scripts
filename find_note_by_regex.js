var findNoteByRegex = function(noteRegex, noteType) {
  ...
  if (noteType === 'host' || noteType === 'all') {
    ...
    Hosts.find({
      ...
      $or: [
	{'notes': {$elemMatch: {'title': {$regex: noteRegex, $options: 'i'}}}},
	{'notes': {$elemMatch: {'content': {$regex: noteRegex, $options: 'i'}}}}]}, {notes: 1}).fetch().forEach(function(host) {
  ...
  if (noteType === 'service' || noteType === 'all') {
    ...
    Ports.find({
      ...
      $or: [
	{'notes': {$elemMatch: {'title': {$regex: noteRegex, $options: 'i'}}}},
	{'notes': {$elemMatch: {'content': {$regex: noteRegex, $options: 'i'}}}}]}, {notes: 1}).fetch().forEach(function(port) {
  ...
  if (noteType === 'vulnerability' || noteType === 'all') {
    ...
    Vulnerabilities.find({
      'project_id' : PROJECT_ID,
      $or: [
	{'evidence': {$regex: noteRegex, $options: 'i'}},
	{'notes': {$elemMatch: {'title': {$regex: noteRegex, $options: 'i'}}}},
	{'notes': {$elemMatch: {'content': {$regex: noteRegex, $options: 'i'}}}}]}, {notes: 1}).fetch().forEach(function(vuln) {
